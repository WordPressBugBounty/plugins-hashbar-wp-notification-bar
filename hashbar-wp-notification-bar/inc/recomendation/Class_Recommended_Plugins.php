...
class HTRP_Recommended_Plugins {
    ...
    function __construct( $args ) {
        ...
        $this->menu_label        =  !empty( $args['menu_label'] ) ? $args['menu_label'] : esc_html__( 'Recommendations', 'hashbar' );
        $this->menu_capability   =  !empty( $args['menu_capability'] ) ? $args['menu_capability'] : 'manage_options';
        $this->menu_page_slug    =  !empty( $args['menu_page_slug'] ) ? $args['menu_page_slug'] : 'hashbar_extensions';
        ...
        // Ajax Action
        add_action( 'wp_ajax_hashbar_ajax_plugin_activation', [ $this, 'plugin_activation' ] );
    ...
    public function enqueue_assets( $hook_suffix ) {
        ...
        $localize_vars['text_domain'] = sanitize_title_with_dashes( 'hashbar' );
        $localize_vars['buttontxt'] = array(
            ...
            'active'     => esc_html__( 'Activated', 'hashbar' ),
        );
                        ...
                        echo '<div id="'.esc_attr( sanitize_title_with_dashes( $tab['title'] ) ).'" class="htrp-admin-tab-pane '.esc_attr( $active_class ).'">';
                            foreach( $plugins as $plugin ):
                                ...
                                if( array_key_exists( $plugin['slug'], $prepare_plugin ) ){
                                    ...
                                    $author_name    = esc_html__( 'HasTheme', 'hashbar' );
                                    $author_link    = !empty( $plugin['author_link'] ) ? $plugin['author_link'] : 'https://www.hasthemes.com';
                                    $details_link   = !empty( $plugin['details_link'] ) ? $plugin['details_link'] : 'https://www.hasthemes.com';
                                    $button_text    = esc_html__('Buy Now', 'hashbar' );
                                ...
                                if ( ! is_wp_error( $data ) ):
                                    ...
                                    if ( file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && is_plugin_inactive( $data['location'] ) ) {
                                        ...
                                        $button_text    = esc_html__( 'Activate', 'hashbar' );

                                    // Not Installed.
                                    } elseif ( ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) ) {

                                        $button_classes = 'button install-now';
                                        $button_text    = esc_html__( 'Install Now', 'hashbar' );

                                    // Active.
                                    } else {
                                        $button_classes = 'button disabled';
                                        $button_text    = esc_html__( 'Activated', 'hashbar' );
                                    }
                                    ...
                                    <div class="plugin-card htrp-plugin-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
                                        <div class="plugin-card-top">
                                            ...
                                            <div class="desc column-description" style="margin-right: 0;">
                                                ...
                                                <p class="authors">
                                                    <cite><?php echo esc_html__( 'By ', 'hashbar' ); ?>
                                        ...
                                        <div class="plugin-card-bottom">
                                            <div class="column-updated">
                                                <?php
                                                    if (! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && $plugins_type == 'pro' ) {
                                                        echo '<a class="button button-primary" href="'.esc_url( $details_link ).'" target="'.esc_attr( $target ).'">'.esc_html__( 'Buy Now', 'hashbar' ).'</a>';
                                                    }else{
                                                ?>
                                            ...
                                            <div class="column-downloaded">
                                                <a href="<?php echo esc_url( $details_link ) ?>" target="<?php echo esc_attr( $target ) ?>" <?php echo esc_html($modal_class); ?>><?php echo esc_html__('More Details', 'hashbar') ?></a>
    ...
    public function plugin_activation() {
        ...
        if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['location'] ) || ! $_POST['location'] ) { // phpcs:ignore
            wp_send_json_error(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Plugin Not Found', 'hashbar' ),
                )
            );
        }

        $plugin_location = ( isset( $_POST['location'] ) ) ? esc_attr( $_POST['location'] ) : ''; // phpcs:ignore
        ...
        wp_send_json_success(
            array(
                'success' => true,
                'message' => esc_html__( 'Plugin Successfully Activated', 'hashbar' ),
            )
        );
